...
adaptive_doa_ula_impl::adaptive_doa_ula_impl(float mu,
    ...
    : gr::sync_block(
          "adaptive_doa_ula",
          // 5 entrées complexes (une par antenne)
          gr::io_signature::make(5, 5, sizeof(gr_complex)),
          // 2 sorties : float scalaire + complex vlen=5
    ...
    , d_last_report(std::chrono::steady_clock::now())
{
    ...
    // Forcer des blocs de 1024 échantillons (comme le Python)
...
float adaptive_doa_ula_impl::get_theta() const
{
    ...
    if      (val >  SIN_MAX) val =  SIN_MAX;
    else if (val < -SIN_MAX) val = -SIN_MAX;
...
{
    // Pointeurs d'entrée (une par antenne)
    ...
    // Copie locale de l'état (évite les accès mémoire répétés)
    double   k_spa = d_k_spa;
    float    mu    = d_mu;
    float    eta   = d_eta;

    // Copie G et W sur la pile (M=5, 4 octets×2 = 10 ou 8 gr_complex)
    gr_complex G[M], W[M-1];
    for (int i = 0; i < M;   ++i) G[i] = d_G[i];
    for (int i = 0; i < M-1; ++i) W[i] = d_W[i];
    ...
    for (int t = 0; t < noutput_items; ++t) {
        ...
        // ── 4. Branches bloquantes Xb[i] = Xs[i] - y ───────────────
        gr_complex Xb[M-1];
        for (int i = 0; i < M-1; ++i)
            Xb[i] = Xs[i] - y;
        ...
        const gr_complex csh   = std::conj(s_hat);  // réutilisé 3 fois

        // ── 7. Mise à jour LMS des poids W ───────────────────────────
        //     W[i] -= eta * conj(s_hat) * Xb[i]
        for (int i = 0; i < M-1; ++i)
            W[i] -= eta * csh * Xb[i];

        // ── 8. Mise à jour gradient du vecteur directeur G ──────────
        //     G[i] += mu * (X[i] - G[i]*s_hat) * conj(s_hat)
        for (int i = 0; i < M; ++i)
            G[i] += mu * (X[i] - G[i] * s_hat) * csh;

        // ── 9. Résidu de phase (OPTIMISÉ : 1 seul sincos) ───────────
        //  On a besoin de exp(-j*x[i]*k_spa) pour x=[-2,-1,0,1,2]
        //  Soit ek1=exp(j*k), ek2=exp(j*2k) :
        //    x=-2 → exp(j*2k)   = ek2
        //    x=-1 → exp(j*k)    = ek1
        //    x= 0 → 1
        //    x=+1 → exp(-j*k)   = conj(ek1)
        //    x=+2 → exp(-j*2k)  = conj(ek2)
        {
            ...
            // phase_sum = Σ x[i] * imag(G[i] * ephase[i])
        ...
        if      (val_sin >  SIN_MAX) val_sin =  SIN_MAX;
        else if (val_sin < -SIN_MAX) val_sin = -SIN_MAX;

        const float theta_hat =
            static_cast<float>(std::asin(val_sin) * (180.0 / M_PI));

        // ── 12. Resynchronisation k_spa + reconstruction G ──────────
        //  Même astuce : 1 seul sincos pour les 5 éléments de G
        k_spa = KD * val_sin;
        {
            double ck = std::cos(k_spa), sk = std::sin(k_spa);
            ...
            const gr_complex ek2n(static_cast<float>(ck*ck - sk*sk),
                                   static_cast<float>(2.0*ck*sk));
            // rebuild_G est inline — zéro overhead
            rebuild_G(ek1n, ek2n, *reinterpret_cast<std::array<gr_complex,M>*>(G));
        }

        // ── Écriture des sorties (convention : −θ et −G comme Python) ─
    ...
    d_k_spa = k_spa;
    for (int i = 0; i < M;   ++i) d_G[i] = G[i];
    for (int i = 0; i < M-1; ++i) d_W[i] = W[i];
